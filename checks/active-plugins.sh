...
run_active_plugins_check() {
    ...
    if [[ ! -f "${plugins_baseline}" || ! -f "${theme_baseline}" ]]; then
        log ERROR "No active-plugins/theme baseline for '${site_name}'. Run cl-wp-sentinel-update-baseline --site=${site_name} first."
        return 1
    fi
...
${plugin_list}"
    ...
    if [[ ${wp_theme_exit} -ne 0 ]]; then
        ...
        if [[ -z "${baseline_theme}" ]]; then
            ...
            log WARN "Theme baseline is empty for '${site_name}' — run cl-wp-sentinel-update-baseline --site=${site_name} to fix"
